...
const TrendingMovieList = ({ list, tittle }) => {
    ...
    return (
        <div className="px-4 w-full relative z-10">
            ...
            <div className="flex space-x-4 overflow-x-auto scrollbar-hide pb-10 relative z-10">

                {list2?.map((movie, index) => (
                    <motion.div
                        key={movie.id}
                        className="flex flex-col items-center justify-start bg-black p-3 shadow-md flex-shrink-0"
                        whileHover={{
                            scale: 1.05,
